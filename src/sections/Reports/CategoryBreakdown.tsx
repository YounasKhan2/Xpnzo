...
const CategoryBreakdown: React.FC<CategoryBreakdownProps> = ({ data }) => {
  ...
  if (data.length === 0) {
    return (
      <Card padding="none" className="h-full flex items-center justify-center p-8 min-h-[300px]">
        <p className="text-text-muted text-center">No spending data for this period.</p>
      </Card>
    );
  }
  ...
  return (
    <Card padding="none" className="h-full flex flex-col overflow-hidden">
      <div className="p-6 border-b border-border">
        <h3 className="text-lg font-bold text-text-primary m-0">Category Breakdown</h3>
      </div>
            ...
            return (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-base font-bold text-text-primary truncate m-0">{item.name}</p>
                  <p className="text-sm text-text-muted m-0 mt-0.5">{percentage}% of total</p>
                </div>
